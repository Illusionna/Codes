...
class PROCESSING:
    ...
    def AbnormalDetect(self, delete:bool=True, show:bool=True) -> None:
        remark = []
        ...
        if show == True:
            if len(remark) >= 1:
                ...
                print(remark)
        if ((delete == True) & (len(remark) >= 1)):
            pos = 0
            for s in range(0, len(remark), 1):
                del self.data[remark[s] - pos]
                pos = -~pos
            print('异常样本已删除...\n')
        else:
            print('无异常样本...\n')
    ...
    def Save(self) -> None:
        if len(self.label) == 0:
            ...
            del temp
        print('结果已保存至 "./Register/ProcessedData/data.xlsx".\n')