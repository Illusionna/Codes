...
def GetLearnRate(optimizer):
    for param_group in optimizer.param_groups:
        return param_group['lr']

def FitOneEpoch(model_train, model, loss, optimizer, epoch, epoch_step, epoch_step_val, gen, genval, Epoch, cuda):
    total_loss = 0
    total_accuracy = 0
    val_loss = 0
    ...
    with tqdm(total=epoch_step_val, desc=f'Epoch {epoch + 1}/{Epoch}', postfix=dict, mininterval=0.3) as pbar:
        for iteration,batch in enumerate(genval):
            ...
            with torch.no_grad():
                ...
                accuracy = torch.mean(equal.float())
            val_loss = val_loss + output.item()
            val_total_accuracy = val_total_accuracy + accuracy.item()
            pbar.set_postfix(
                **{
    ...
    torch.save(
        model.state_dict(),
        './Logs/log%03d.pth' % (epoch + 1)
    )
    print(f'log {epoch} loss: {total_loss / epoch_step}')
    print(f'log {epoch+1} val loss: {val_loss / epoch_step_val}')