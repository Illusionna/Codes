...
def Gmm(matrix:list, n_clusters:int=3) -> tuple:
    model = GMM(n_clusters)
...
def Predict(cluster:object, vector:list, judge:str) -> None:
    if judge == 'single':
        ...
        print('Predicted results:', cluster.predict(vector))

def Iteration(xlsx:str, matrix:list, epoch:int=3, flush=False) -> None:
    bar = [i for i in range(2,epoch+2,1)]
    io = f'./Cluster/Programs/GMM/Results/{xlsx}/GMMIntrinsicIndex'
    ios = io + '/Index.txt'
    ...
    file.close()
    iot = f'./Cluster/Programs/GMM/Results/{xlsx}/GMMIteration'
    ...
    for n in tqdm(bar, desc='Iteration', ncols=60,mininterval=1e-5):
        ...
        if len(cluster.predict(data)) <= 120:
            print('Cluster result label:')
            print(cluster.predict(data))
        else:
            print('Cluster result label:')
        ...
        for j in range(0, n, 1):
            for t in range(0, len(cluster.predict(data)), 1):
                value = cluster.predict(data)[t]
                if value == j:
                    f.write(str(data[t]))
                    f.write('\n')
                else:
...
def predictInterface(predictedData, predictedDataType, n_clusters) -> None:
    (cluster,INDEX) = Gmm(data, n_clusters=n_clusters)
    Predict(cluster, predictedData, judge=predictedDataType)

def GMMInterface(io, maxEpoch, judgeStandardization, judgeNormalization, accuracy, floatType, judgeRandomShuffle, judgeFlush, judgePredict, predictedData, predictedDataType, n_clusters) -> None:
...
if __name__ == '__main__':
    ...
    # # judge = {'single', 'multi'}.
    (cluster,INDEX) = Gmm(data, n_clusters=5)
