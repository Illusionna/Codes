...
def saveSOM(clusteringSOM:list, n_clusters:int, judge:bool=True) -> None:
    if judge == True:
        ...
        os.makedirs("./SOM/Result", exist_ok=True)
        SOMfile = open("./SOM/Result/GMMResults.txt", mode = "w", encoding = "utf-8")
    elif judge == False:
        SOMfile = open(f"./SOM/IterationClusterResults/Iteration{n_clusters-1}_SOM_Cluster_Result.txt", mode = "w", encoding = "utf-8")
    clusterResultList = []
...
def SOM_Iteration(epoch:int, judge:bool=True) -> None:
    ...
    if judge == True:
        ...
        os.makedirs("./SOM/IterationClusterResults", exist_ok=True)
        # 在 OptimalClusterResult 文件夹放置每轮聚类内在指数，并返回最优聚类数.
        os.makedirs("./SOM/OptimalClusterResult", exist_ok=True)
        # 内在聚类指数 DBI 和 DI 结果存至 Intrinsic_Exponential.txt 文件里，先取定 mode = "w"，清空 txt 文件，再取 mode = "a"，文本以追加形式存入 txt 文件.
        SOMFile = open("./SOM/OptimalClusterResult/Intrinsic_Exponential.txt", mode = "w", encoding = "utf-8")
        SOMFile = open("./SOM/OptimalClusterResult/Intrinsic_Exponential.txt", mode = "a", encoding = "utf-8")
        #*****************************
        matrixSOM = []
